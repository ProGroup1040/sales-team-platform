...
export const projects = mysqlTable("projects", {
  ...
  // التعاقد لا يعني بدء التنفيذ. هذه البيانات تحدد متى يبدأ الـ SLA فعلياً.
  preExecutionStatus: varchar("preExecutionStatus", { length: 96 }).notNull().default("waiting_site_readiness"),
  preExecutionWaitingOwnerCode: varchar("preExecutionWaitingOwnerCode", { length: 64 }).default("client"),
  preExecutionWaitingReasonCode: varchar("preExecutionWaitingReasonCode", { length: 96 }).default("site_not_ready"),
  preExecutionNotes: text("preExecutionNotes"),
  expectedSiteReadyDate: date("expectedSiteReadyDate"),
  siteReadyDate: date("siteReadyDate"),
  siteReadySource: varchar("siteReadySource", { length: 64 }),
  siteReadyRecordedBy: varchar("siteReadyRecordedBy", { length: 120 }),
  siteReadyRecordedAt: timestamp("siteReadyRecordedAt"),
  siteReadyNotes: text("siteReadyNotes"),
  executionSurveyRequestedDate: date("executionSurveyRequestedDate"),
  executionSurveyScheduledDate: date("executionSurveyScheduledDate"),
  executionSurveyActualDate: date("executionSurveyActualDate"),
  executionSurveyStatus: varchar("executionSurveyStatus", { length: 64 }).notNull().default("not_requested"),
  executionSurveyEngineerId: int("executionSurveyEngineerId"),
  executionSurveyNotes: text("executionSurveyNotes"),
  executionStartDate: date("executionStartDate"),
  executionStartApprovedBy: varchar("executionStartApprovedBy", { length: 120 }),
  executionStartApprovedAt: timestamp("executionStartApprovedAt"),
  standardExecutionDays: int("standardExecutionDays").notNull().default(45),
  executionClockStatus: varchar("executionClockStatus", { length: 32 }).notNull().default("not_started"),
  ...
  closingStatus: varchar("closingStatus", { length: 64 }),
  closingOtherDescription: text("closingOtherDescription"),
  closingNotes: text("closingNotes"),
  closedBy: varchar("closedBy", { length: 120 }),
  closedAt: timestamp("closedAt"),
  previousStageBeforeClose: varchar("previousStageBeforeClose", { length: 64 }),
  previousStatusBeforeClose: varchar("previousStatusBeforeClose", { length: 32 }),
...
export const projectMovements = mysqlTable("project_movements", {
  ...
  previousResponsibleId: int("previousResponsibleId"),
  newResponsibleId: int("newResponsibleId"),
  assignedAt: timestamp("assignedAt"),
  assignedBy: varchar("assignedBy", { length: 120 }),
  enteredAt: timestamp("enteredAt").defaultNow().notNull(),
  plannedCompletionDate: date("plannedCompletionDate"),
...
export const projectUpdates = mysqlTable("project_updates", {
  ...
  currentStageName: varchar("currentStageName", { length: 120 }),
  currentDepartment: varchar("currentDepartment", { length: 120 }),
  currentResponsibleId: int("currentResponsibleId"),
  currentResponsibleName: varchar("currentResponsibleName", { length: 120 }),
  salesOwnerId: int("salesOwnerId"),
  salesOwnerName: varchar("salesOwnerName", { length: 120 }),
  daysInCurrentStage: int("daysInCurrentStage").notNull().default(0),
  plannedExitDate: date("plannedExitDate"),
  stageDelayDays: int("stageDelayDays").notNull().default(0),
  inheritedDelayDays: int("inheritedDelayDays").notNull().default(0),
  totalDelayDays: int("totalDelayDays").notNull().default(0),
  newDelaySinceLastUpdate: int("newDelaySinceLastUpdate").notNull().default(0),
  nextAction: text("nextAction"),

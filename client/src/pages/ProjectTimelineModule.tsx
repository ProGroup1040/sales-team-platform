...
const STAGE_DELAY_OWNER_CODES: Record<string, string[]> = {
...
function CompactPreExecutionPanel({ project, engineers, updatedBy, onChanged }: { project: any; engineers: any[]; updatedBy: string; onChanged: () => Promise<void> }) {
  const legacySurveyStatus = project.executionSurveyStatus === "completed" ? "completed" : project.executionSurveyStatus === "scheduled" ? "scheduled" : project.executionSurveyStatus === "cancelled" ? "not_done" : project.executionSurveyStatus === "resurvey_required" ? "resurvey_required" : "unspecified";
  const [waitingStatus, setWaitingStatus] = useState("waiting_site_readiness");
  const [waitingOwner, setWaitingOwner] = useState("client");
  const [plannedSurveyDate, setPlannedSurveyDate] = useState("");
  const [actualSurveyDate, setActualSurveyDate] = useState("");
  const [surveyStatus, setSurveyStatus] = useState("unspecified");
  const [surveyEngineerId, setSurveyEngineerId] = useState("all");
  const [notes, setNotes] = useState("");
  const [startConfirmed, setStartConfirmed] = useState(false);

  useEffect(() => {
    setWaitingStatus(["waiting_site_readiness", "execution_survey_scheduled", "waiting_financial_requirement", "other"].includes(project.preExecutionStatus) ? project.preExecutionStatus : "waiting_site_readiness");
    setWaitingOwner(["client", "sales_engineer", "sales_department", "other"].includes(project.preExecutionWaitingOwnerCode) ? project.preExecutionWaitingOwnerCode : "client");
    setPlannedSurveyDate(toDateInput(project.executionSurveyScheduledDate));
    setActualSurveyDate(toDateInput(project.executionSurveyActualDate));
    setSurveyStatus(legacySurveyStatus);
    setSurveyEngineerId(project.executionSurveyEngineerId ? String(project.executionSurveyEngineerId) : "all");
    setNotes(project.preExecutionNotes ?? "");
  }, [project.id, project.preExecutionStatus, project.preExecutionWaitingOwnerCode, project.executionSurveyScheduledDate, project.executionSurveyActualDate, project.executionSurveyEngineerId, project.executionSurveyStatus, project.preExecutionNotes, legacySurveyStatus]);

  useEffect(() => {
    const legacyTitle = Array.from(document.querySelectorAll("h3")).find((element) => element.textContent?.includes("ما قبل التنفيذ · جاهزية الموقع والمعاينة"));
    const legacyCard = legacyTitle?.closest('[class*="border-slate"]') as HTMLElement | null;
    if (legacyCard) legacyCard.style.display = "none";
    return () => { if (legacyCard) legacyCard.style.display = ""; };
  }, [project.id]);

  const update = trpc.projectTimeline.updatePreExecution.useMutation({
    onSuccess: async () => { toast.success("تم حفظ بيانات ما قبل التنفيذ"); await onChanged(); },
    onError: (error) => toast.error(error.message),
  });
  const start = trpc.projectTimeline.startExecution.useMutation({
    onSuccess: async () => { toast.success("تم اعتماد المعاينة وبدء عداد التنفيذ"); setStartConfirmed(false); await onChanged(); },
    onError: (error) => toast.error(error.message),
  });

  const otherNeedsExplanation = waitingStatus === "other" || waitingOwner === "other";
  const scheduleRequired = waitingStatus === "execution_survey_scheduled" || surveyStatus === "scheduled";
  const completionRequired = surveyStatus === "completed";
  const surveyComplete = surveyStatus === "completed" && !!actualSurveyDate && surveyEngineerId !== "all";
  const saveDisabled = update.isPending || (otherNeedsExplanation && !notes.trim()) || (scheduleRequired && !plannedSurveyDate) || (completionRequired && (!actualSurveyDate || surveyEngineerId === "all"));

  return (
    <Card className="border-slate-400/30 bg-slate-500/[0.04]">
      <CardHeader><CardTitle className="text-base">ما قبل التنفيذ · متابعة الجاهزية والمعاينة</CardTitle></CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1"><Label>حالة الانتظار</Label><Select value={waitingStatus} onValueChange={setWaitingStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="waiting_site_readiness">بانتظار جاهزية الموقع</SelectItem><SelectItem value="execution_survey_scheduled">تم تحديد المعاينة التنفيذية</SelectItem><SelectItem value="waiting_financial_requirement">بانتظار متطلب مادي</SelectItem><SelectItem value="other">أخرى</SelectItem></SelectContent></Select></div>
          <div className="space-y-1"><Label>مسؤول الانتظار</Label><Select value={waitingOwner} onValueChange={setWaitingOwner}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="client">العميل</SelectItem><SelectItem value="sales_engineer">مهندس المبيعات</SelectItem><SelectItem value="sales_department">إدارة المبيعات</SelectItem><SelectItem value="other">أخرى</SelectItem></SelectContent></Select></div>
        </div>
        {(waitingStatus === "execution_survey_scheduled" || surveyStatus === "scheduled" || surveyStatus === "completed" || surveyStatus === "not_done" || surveyStatus === "resurvey_required") && <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1"><Label>موعد المعاينة التنفيذية</Label><Input type="date" value={plannedSurveyDate} onChange={(event) => setPlannedSurveyDate(event.target.value)} /></div><div className="space-y-1"><Label>مهندس المعاينة</Label><Select value={surveyEngineerId} onValueChange={setSurveyEngineerId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">غير محدد</SelectItem>{engineers.map((engineer) => <SelectItem key={engineer.id} value={String(engineer.id)}>{engineer.name}</SelectItem>)}</SelectContent></Select></div></div>}
        <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1"><Label>حالة المعاينة</Label><Select value={surveyStatus} onValueChange={setSurveyStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="unspecified">غير محددة</SelectItem><SelectItem value="scheduled">تم تحديدها</SelectItem><SelectItem value="completed">تمت</SelectItem><SelectItem value="not_done">لم تتم</SelectItem><SelectItem value="resurvey_required">إعادة معاينة مطلوبة</SelectItem></SelectContent></Select></div>{surveyStatus === "completed" && <div className="space-y-1"><Label>تاريخ تنفيذ المعاينة فعلياً</Label><Input type="date" value={actualSurveyDate} onChange={(event) => setActualSurveyDate(event.target.value)} /></div>}</div>
        <div className="space-y-1"><Label>{otherNeedsExplanation ? "توضيح *" : "سبب الانتظار وملاحظات الموقع"}</Label><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="لماذا المشروع لم يبدأ أو ما المطلوب قبل التنفيذ؟" /></div>
        <div className="grid gap-2 sm:grid-cols-2"><Button variant="outline" disabled={saveDisabled} onClick={() => update.mutate({ projectId: project.id, preExecutionStatus: waitingStatus as any, waitingOwnerCode: waitingOwner as any, waitingReasonCode: waitingStatus, notes: notes || undefined, surveyScheduledDate: plannedSurveyDate || undefined, surveyActualDate: actualSurveyDate || undefined, surveyStatus: surveyStatus as any, surveyEngineerId: surveyEngineerId === "all" ? undefined : Number(surveyEngineerId), updatedBy })}>{update.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}حفظ ما قبل التنفيذ</Button><Button disabled={start.isPending || !surveyComplete} onClick={() => { if (window.confirm("سيبدأ الآن عداد التنفيذ لمدة 45 يوماً من تاريخ اليوم المحدد. هل تريد المتابعة؟")) start.mutate({ projectId: project.id, notes: notes || undefined, updatedBy }); }}>{start.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <TimerReset className="ml-2 h-4 w-4" />}اعتماد وبدء التنفيذ</Button></div>
        <p className="text-[11px] leading-5 text-muted-foreground">لا يبدأ عداد 45 يوماً ولا يدخل المشروع في تأخيرات التنفيذ قبل إتمام المعاينة وتسجيل تاريخها الفعلي ومهندس المعاينة.</p>
      </CardContent>
    </Card>
  );
}

function CurrentStageOverview({ project, stages, engineers }: { project: any; stages: any[]; engineers: any[] }) {
  const stage = stages.find((item) => item.stageKey === project.currentStageKey);
  const responsible = engineers.find((item) => item.id === project.currentResponsibleId);
  const enteredAt = project.currentMovement?.enteredAt ?? project.currentStageEnteredAt;
  const elapsed = enteredAt ? Math.max(0, Math.floor((Date.now() - new Date(String(enteredAt)).getTime()) / 86400000)) : 0;
  return <Card className="border-primary/25 bg-primary/[.035]"><CardHeader><CardTitle className="text-base">بيانات المرحلة الحالية</CardTitle></CardHeader><CardContent className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3"><div><p className="text-xs text-muted-foreground">المرحلة الحالية</p><p className="mt-1 font-bold">{stage?.nameAr ?? project.currentStageKey}</p><p className="text-xs text-muted-foreground">{project.currentDepartment}</p></div><div><p className="text-xs text-muted-foreground">المسؤول الحالي</p><p className="mt-1 font-bold">{responsible?.name ?? "غير محدد"}</p><p className="text-xs text-muted-foreground">مهندس المبيعات: {project.salesEngineerName ?? "—"}</p></div><div><p className="text-xs text-muted-foreground">تاريخ الدخول · SLA</p><p className="mt-1 font-bold">{formatDate(enteredAt)} · {project.currentMovement?.slaDays ?? stage?.defaultSlaDays ?? 0} يوم</p><p className="text-xs text-muted-foreground">داخل المرحلة: {elapsed} يوم</p></div><div><p className="text-xs text-muted-foreground">هل يوجد تأخير؟</p><Badge variant="outline" className={project.currentStageDelayDays > 0 ? "mt-1 border-rose-400/40 bg-rose-500/10 text-rose-400" : "mt-1 border-emerald-400/40 bg-emerald-500/10 text-emerald-400"}>{project.currentStageDelayDays > 0 ? `نعم · ${project.currentStageDelayDays} يوم` : "لا"}</Badge></div></CardContent></Card>;
}

function ScheduleUpdateCard({ project, updatedBy, onChanged }: { project: any; updatedBy: string; onChanged: () => Promise<void> }) {
  const [nextAction, setNextAction] = useState(""); const [expectedDate, setExpectedDate] = useState(""); const [hasBlocker, setHasBlocker] = useState("no"); const [blockerDescription, setBlockerDescription] = useState(""); const [notes, setNotes] = useState("");
  useEffect(() => { setNextAction(project.nextRequiredAction ?? ""); setExpectedDate(toDateInput(project.expectedProjectCompletionDate)); setHasBlocker("no"); setBlockerDescription(""); setNotes(""); }, [project.id, project.nextRequiredAction, project.expectedProjectCompletionDate]);
  useEffect(() => { const title = Array.from(document.querySelectorAll("h3")).find((element) => element.textContent?.includes("تحديث الاثنين / الأربعاء")); const card = title?.closest('[class*="border-border"]') as HTMLElement | null; if (card) card.style.display = "none"; return () => { if (card) card.style.display = ""; }; }, [project.id]);
  const mutation = trpc.projectTimeline.update.useMutation({ onSuccess: async () => { toast.success("تم حفظ تحديث المواعيد"); await onChanged(); }, onError: (error) => toast.error(error.message) });
  const blocker = hasBlocker === "yes";
  return <Card className="border-border/70"><CardHeader><CardTitle className="text-base">تحديث المواعيد</CardTitle></CardHeader><CardContent className="space-y-3"><div className="space-y-1"><Label>الإجراء التالي</Label><Input value={nextAction} onChange={(event) => setNextAction(event.target.value)} placeholder="مثال: استلام اعتماد العميل للتصميم" /></div><div className="space-y-1"><Label>تاريخ الإكمال المتوقع</Label><Input type="date" value={expectedDate} onChange={(event) => setExpectedDate(event.target.value)} /></div><div className="space-y-1"><Label>هل يوجد عائق؟</Label><Select value={hasBlocker} onValueChange={setHasBlocker}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="no">لا</SelectItem><SelectItem value="yes">نعم</SelectItem></SelectContent></Select></div>{blocker && <div className="space-y-1"><Label>العائق الحالي *</Label><Textarea value={blockerDescription} onChange={(event) => setBlockerDescription(event.target.value)} placeholder="اكتب العائق الحالي" /></div>}<div className="space-y-1"><Label>ملاحظات التحديث</Label><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="تفاصيل إضافية" /></div><Button className="w-full" disabled={mutation.isPending || (blocker && !blockerDescription.trim())} onClick={() => mutation.mutate({ projectId: project.id, currentStatus: project.projectStatus, nextAction: nextAction || undefined, expectedCompletionDate: expectedDate || undefined, hasBlocker: blocker, blockerDescription: blocker ? blockerDescription : undefined, notes: notes || undefined, updatedBy })}>{mutation.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}حفظ التحديث</Button></CardContent></Card>;
}

function ProjectCloseCard({ project, updatedBy, onChanged }: { project: any; updatedBy: string; onChanged: () => Promise<void> }) {
  const [open, setOpen] = useState(false);
  const [closingStatus, setClosingStatus] = useState("installed");
  const [closingDate, setClosingDate] = useState(() => new Date().toISOString().slice(0, 10));
  const [otherDescription, setOtherDescription] = useState("");
  const [notes, setNotes] = useState("");
  useEffect(() => {
    setClosingStatus("installed"); setClosingDate(new Date().toISOString().slice(0, 10)); setOtherDescription(""); setNotes(""); setOpen(false);
  }, [project.id]);
  const mutation = trpc.projectTimeline.close.useMutation({
    onSuccess: async () => { toast.success("تم إغلاق المشروع واستبعاده من المتابعة التشغيلية"); setOpen(false); await onChanged(); },
    onError: (error) => toast.error(error.message),
  });
  const canConfirm = !!closingDate && (closingStatus !== "other" || !!otherDescription.trim());
  return <Card className="border-emerald-500/30 bg-emerald-500/[0.035]"><CardHeader><CardTitle className="text-base">إغلاق المشروع</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm leading-6 text-muted-foreground">استخدم هذا الإجراء فقط عند اكتمال التركيب أو التسليم وانتهاء المتابعة التشغيلية. لن تُحذف البيانات التاريخية.</p><Button className="w-full bg-emerald-600 hover:bg-emerald-700" onClick={() => setOpen(true)}><CheckCircle2 className="ml-2 h-4 w-4" />تم اكتمال المشروع وإغلاقه</Button><Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>تأكيد إغلاق المشروع</DialogTitle><DialogDescription>هل أنت متأكد أن المشروع تم تنفيذه وانتهت متابعته التشغيلية؟ بعد الإغلاق سيختفي من Project Timeline التشغيلي.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="space-y-1"><Label>حالة الإغلاق *</Label><Select value={closingStatus} onValueChange={setClosingStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="installed">تم التركيب</SelectItem><SelectItem value="delivered">تم التسليم</SelectItem><SelectItem value="execution_completed">تم اكتمال التنفيذ</SelectItem><SelectItem value="final_closed">تم الإغلاق النهائي</SelectItem><SelectItem value="other">أخرى</SelectItem></SelectContent></Select></div>{closingStatus === "other" && <div className="space-y-1"><Label>توضيح حالة الإغلاق *</Label><Input value={otherDescription} onChange={(event) => setOtherDescription(event.target.value)} placeholder="اكتب وصف الحالة" /></div>}<div className="space-y-1"><Label>تاريخ الإغلاق *</Label><Input type="date" value={closingDate} onChange={(event) => setClosingDate(event.target.value)} /></div><div className="space-y-1"><Label>ملاحظات الإغلاق</Label><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="ملاحظات اختيارية" /></div><Button className="w-full bg-emerald-600 hover:bg-emerald-700" disabled={mutation.isPending || !canConfirm} onClick={() => mutation.mutate({ projectId: project.id, closingStatus: closingStatus as any, closingOtherDescription: closingStatus === "other" ? otherDescription.trim() : undefined, closingDate, closingNotes: notes.trim() || undefined, updatedBy })}>{mutation.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}تأكيد الإغلاق</Button></div></DialogContent></Dialog></CardContent></Card>;
}

function StageDelayCard({ project, owners, reasons, updatedBy, onChanged }: { project: any; owners: any[]; reasons: any[]; updatedBy: string; onChanged: () => Promise<void> }) {
  const [hasDelay, setHasDelay] = useState("no"); const [days, setDays] = useState(""); const [ownerCode, setOwnerCode] = useState("all"); const [reasonCode, setReasonCode] = useState("all"); const [otherReason, setOtherReason] = useState(""); const [notes, setNotes] = useState("");
  useEffect(() => { setHasDelay("no"); setDays(""); setOwnerCode("all"); setReasonCode("all"); setOtherReason(""); setNotes(""); }, [project.id]);
  useEffect(() => { const title = Array.from(document.querySelectorAll("h3")).find((element) => element.textContent?.includes("إضافة تأخير إلى Delay Ledger")); const card = title?.closest('[class*="border-border"]') as HTMLElement | null; if (card) card.style.display = "none"; return () => { if (card) card.style.display = ""; }; }, [project.id]);
  const stageOwners = owners.filter((owner) => (STAGE_DELAY_OWNER_CODES[project.currentStageKey] ?? owners.map((item) => item.code)).includes(owner.code));
  const availableReasons = [...reasons.filter((reason) => reason.ownerCode === ownerCode), { code: "other", labelAr: "أخرى" }].filter((reason, index, array) => array.findIndex((item) => item.code === reason.code) === index);
  const mutation = trpc.projectTimeline.addDelay.useMutation({ onSuccess: async () => { toast.success("تم تسجيل التأخير"); await onChanged(); }, onError: (error) => toast.error(error.message) });
  const show = hasDelay === "yes"; const canSave = show && Number(days) > 0 && ownerCode !== "all" && reasonCode !== "all" && (reasonCode !== "other" || otherReason.trim());
  return <Card className="border-border/70"><CardHeader><CardTitle className="text-base">تأخير المرحلة الحالية</CardTitle></CardHeader><CardContent className="space-y-3"><div className="space-y-1"><Label>هل يوجد تأخير؟</Label><Select value={hasDelay} onValueChange={setHasDelay}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="no">لا</SelectItem><SelectItem value="yes">نعم</SelectItem></SelectContent></Select></div>{show && <><div className="space-y-1"><Label>عدد أيام التأخير</Label><Input type="number" min="1" value={days} onChange={(event) => setDays(event.target.value)} /></div><div className="space-y-1"><Label>مسؤول التأخير</Label><Select value={ownerCode} onValueChange={(value) => { setOwnerCode(value); setReasonCode("all"); }}><SelectTrigger><SelectValue placeholder="اختر المسؤول" /></SelectTrigger><SelectContent>{stageOwners.map((owner) => <SelectItem key={owner.code} value={owner.code}>{owner.labelAr}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1"><Label>سبب التأخير</Label><Select value={reasonCode} onValueChange={setReasonCode}><SelectTrigger><SelectValue placeholder="اختر السبب" /></SelectTrigger><SelectContent>{ownerCode !== "all" && availableReasons.map((reason) => <SelectItem key={reason.code} value={reason.code}>{reason.labelAr}</SelectItem>)}</SelectContent></Select></div>{reasonCode === "other" && <div className="space-y-1"><Label>حدد سبب التأخير *</Label><Input value={otherReason} onChange={(event) => setOtherReason(event.target.value)} placeholder="اكتب السبب" /></div>}<div className="space-y-1"><Label>ملاحظات التأخير</Label><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="تفاصيل اختيارية" /></div><Button className="w-full" variant="destructive" disabled={mutation.isPending || !canSave} onClick={() => mutation.mutate({ projectId: project.id, movementId: project.currentMovement?.id, delayDays: Number(days), ownerCode, reasonCode, notes: reasonCode === "other" ? `سبب آخر: ${otherReason}${notes ? `\n${notes}` : ""}` : notes || undefined, createdBy: updatedBy })}>{mutation.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <TriangleAlert className="ml-2 h-4 w-4" />}تسجيل التأخير</Button></>}</CardContent></Card>;
}
...
export default function ProjectTimelineModule() {
  ...
  const [preExecutionStatus, setPreExecutionStatus] = useState<any>("waiting_site_readiness");
  const [waitingOwnerCode, setWaitingOwnerCode] = useState<any>("client");
            ...
            {listQuery.isLoading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : projects.length === 0 ? <div className="flex min-h-64 flex-col items-center justify-center gap-3 p-8 text-center"><GitBranch className="h-10 w-10 text-muted-foreground/40" /><div><p className="font-semibold">لا توجد مشاريع مطابقة للفلاتر</p><p className="mt-1 text-sm text-muted-foreground">يمكن للإدارة مزامنة الصفقات المغلقة لإنشاء مشاريعها التشغيلية تلقائياً.</p></div></div> : <div className="overflow-x-auto">
              <table className="w-full min-w-[1360px] text-right text-sm">
                ...
                <tbody className="divide-y divide-border/60">{projects.map((project) => <tr key={project.id} onClick={() => setDetailId(project.id)} className="cursor-pointer transition-colors hover:bg-primary/5">
                  ...
                  <td className="px-4 py-3"><p>{project.currentResponsible?.name ?? "—"}</p><p className="mt-1 text-xs text-muted-foreground">تشغيلياً</p></td>
                  <td className="px-4 py-3">{project.isPreExecution ? <><span className="font-semibold text-slate-300">انتظار: {project.waitingDays} يوم</span><p className="mt-1 text-xs text-muted-foreground">ليس تأخير شركة</p></> : <span className="font-semibold">{project.daysInCurrentStage} يوم</span>}</td>
                  ...
                  <td className="px-4 py-3"><span className={project.currentStageDelayDays > 0 ? "font-bold text-rose-400" : "text-muted-foreground"}>{project.currentStageDelayDays} يوم</span></td>
                  ...
                  <td className="px-4 py-3"><StatusBadge status={project.projectStatus} /><p className="mt-2">{formatDate(project.lastUpdatedAt)}</p><p className={`mt-1 text-xs ${project.updateStatus === "missing" ? "text-rose-400" : "text-emerald-400"}`}>{project.updateStatus === "missing" ? "تحديث مفقود" : "محدّث"}</p></td>
                </tr>)}</tbody>
              </table>
            </div>}
          </CardContent>
          ...
          {detailQuery.isLoading ? <div className="flex min-h-80 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : !detail ? <div className="p-8 text-center text-muted-foreground">تعذر تحميل تفاصيل المشروع.</div> : <>
            ...
            <div className="space-y-5 p-5 md:p-6">
              ...
              <Tabs defaultValue="timeline" className="w-full">
                <TabsList className={`grid h-auto w-full ${canEditProject ? "grid-cols-4" : "grid-cols-3"} bg-muted/60`}><TabsTrigger value="timeline">المسار الزمني</TabsTrigger><TabsTrigger value="delays">دفتر التأخيرات</TabsTrigger><TabsTrigger value="updates">سجل التحديثات</TabsTrigger>{canEditProject ? <TabsTrigger value="actions">التحديث والإجراءات</TabsTrigger> : null}</TabsList>
                <TabsContent value="updates" className="mt-5 space-y-3">{(detail.updates ?? []).length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">لا توجد تحديثات تاريخية بعد. سيُحفظ Snapshot مستقل عند كل حفظ لتحديث المواعيد.</div> : (detail.updates ?? []).map((update: any) => <Card key={update.id} className="border-border/70"><CardContent className="p-4"><div className="flex flex-col justify-between gap-2 sm:flex-row"><div><p className="font-bold">{formatDate(update.createdAt)} · {new Date(update.createdAt).toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" })}</p><p className="text-xs text-muted-foreground">بواسطة {update.updatedBy}</p></div><Badge variant="outline">تأخير جديد: +{update.newDelaySinceLastUpdate ?? 0} يوم</Badge></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-muted-foreground">المرحلة / الإدارة</p><p className="mt-1 font-medium">{update.currentStageName ?? update.currentStageKey} · {update.currentDepartment ?? "—"}</p></div><div><p className="text-xs text-muted-foreground">المسؤول / المبيعات</p><p className="mt-1 font-medium">{update.currentResponsibleName ?? "—"} / {update.salesOwnerName ?? "—"}</p></div><div><p className="text-xs text-muted-foreground">أيام المرحلة / الخروج</p><p className="mt-1 font-medium">{update.daysInCurrentStage ?? 0} يوم · {formatDate(update.plannedExitDate)}</p></div><div><p className="text-xs text-muted-foreground">تأخير المرحلة / الموروث / الكلي</p><p className="mt-1 font-medium text-rose-400">{update.stageDelayDays ?? 0} / {update.inheritedDelayDays ?? 0} / {update.totalDelayDays ?? 0} يوم</p></div></div><div className="mt-3 grid gap-2 rounded-lg bg-muted/40 p-3 text-xs sm:grid-cols-2"><p><span className="text-muted-foreground">المسؤول والسبب: </span>{update.delayOwnerCode ?? "—"} · {update.delayReasonCode ?? "—"}</p><p><span className="text-muted-foreground">العائق: </span>{update.hasBlocker ? update.blockerDescription ?? "يوجد عائق" : "لا يوجد"}</p><p><span className="text-muted-foreground">الإجراء التالي: </span>{update.nextAction ?? "—"}</p><p><span className="text-muted-foreground">ملاحظات: </span>{update.notes ?? "—"}</p></div></CardContent></Card>)}</TabsContent>
                ...
                <TabsContent value="actions" className="mt-5"><div className="grid gap-5 xl:grid-cols-2">{!selectedProject.executionStartDate ? <Card className="border-slate-400/30 bg-slate-500/[0.04]"><CardHeader><CardTitle className="text-base">ما قبل التنفيذ · جاهزية الموقع والمعاينة</CardTitle></CardHeader><CardContent className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1"><Label>حالة الانتظار</Label><Select value={preExecutionStatus} onValueChange={setPreExecutionStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="waiting_site_readiness">بانتظار جاهزية الموقع</SelectItem><SelectItem value="waiting_execution_survey">بانتظار طلب المعاينة التنفيذية</SelectItem><SelectItem value="execution_survey_scheduled">تم تحديد معاينة تنفيذية</SelectItem><SelectItem value="waiting_client_information">بانتظار بيانات العميل</SelectItem><SelectItem value="waiting_payment_requirement">بانتظار متطلب مالي</SelectItem><SelectItem value="waiting_client_confirmation">بانتظار تأكيد العميل</SelectItem><SelectItem value="other">أخرى</SelectItem></SelectContent></Select></div><div className="space-y-1"><Label>مسؤول الانتظار</Label><Select value={waitingOwnerCode} onValueChange={setWaitingOwnerCode}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{owners.map((owner) => <SelectItem key={owner.code} value={owner.code}>{owner.labelAr}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1"><Label>تاريخ جاهزية الموقع</Label><Input type="date" value={siteReadyDate} onChange={(event) => setSiteReadyDate(event.target.value)} /></div><div className="space-y-1"><Label>مصدر الجاهزية</Label><Select value={siteReadySource} onValueChange={setSiteReadySource}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="client_notification">إبلاغ العميل</SelectItem><SelectItem value="sales_engineer_confirmation">تأكيد مهندس المبيعات</SelectItem><SelectItem value="site_inspection">فحص موقع</SelectItem><SelectItem value="management_confirmation">تأكيد الإدارة</SelectItem></SelectContent></Select></div><div className="space-y-1"><Label>موعد المعاينة</Label><Input type="date" value={surveyScheduledDate} onChange={(event) => setSurveyScheduledDate(event.target.value)} /></div><div className="space-y-1"><Label>تنفيذ المعاينة فعلياً</Label><Input type="date" value={surveyActualDate} onChange={(event) => setSurveyActualDate(event.target.value)} /></div><div className="space-y-1"><Label>حالة المعاينة</Label><Select value={surveyStatus} onValueChange={setSurveyStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="not_requested">لم تُطلب</SelectItem><SelectItem value="requested">تم الطلب</SelectItem><SelectItem value="scheduled">تم التحديد</SelectItem><SelectItem value="completed">تمت</SelectItem><SelectItem value="cancelled">ألغيت</SelectItem></SelectContent></Select></div><div className="space-y-1"><Label>مهندس المعاينة</Label><Select value={surveyEngineerId} onValueChange={setSurveyEngineerId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">غير محدد</SelectItem>{engineers.map((engineer) => <SelectItem key={engineer.id} value={String(engineer.id)}>{engineer.name}</SelectItem>)}</SelectContent></Select></div></div><Textarea value={preExecutionNotes} onChange={(event) => setPreExecutionNotes(event.target.value)} placeholder="سبب الانتظار وملاحظات الموقع. لا يدخل هذا الوقت كتأخير شركة." /><div className="grid gap-2 sm:grid-cols-2"><Button variant="outline" disabled={preExecutionMutation.isPending} onClick={() => detailId && preExecutionMutation.mutate({ projectId: detailId, preExecutionStatus, waitingOwnerCode, waitingReasonCode, notes: preExecutionNotes || undefined, expectedSiteReadyDate: expectedSiteReadyDate || undefined, siteReadyDate: siteReadyDate || undefined, siteReadySource: siteReadySource as any, surveyRequestedDate: surveyRequestedDate || undefined, surveyScheduledDate: surveyScheduledDate || undefined, surveyActualDate: surveyActualDate || undefined, surveyStatus: surveyStatus as any, surveyEngineerId: surveyEngineerId === "all" ? undefined : Number(surveyEngineerId), surveyNotes: surveyNotes || undefined, updatedBy })}>{preExecutionMutation.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}حفظ ما قبل التنفيذ</Button><Button disabled={startExecutionMutation.isPending || !(surveyStatus === "completed" || surveyActualDate)} onClick={() => detailId && startExecutionMutation.mutate({ projectId: detailId, executionStartDate: executionStartDate || undefined, responsibleId: responsibleId === "all" ? undefined : Number(responsibleId), notes: preExecutionNotes || undefined, updatedBy })}>{startExecutionMutation.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <TimerReset className="ml-2 h-4 w-4" />}اعتماد وبدء التنفيذ</Button></div><p className="text-[11px] leading-5 text-muted-foreground">لن يبدأ عداد الـ 45 يوم ولا أي SLA قبل اكتمال المعاينة التنفيذية واعتماد هذا الإجراء.</p></CardContent></Card> : null}<Card className="border-border/70"><CardHeader><CardTitle className="text-base">تحديث الاثنين / الأربعاء</CardTitle></CardHeader><CardContent className="space-y-3"><div className="space-y-1"><Label>الإجراء التالي</Label><Input value={reviewAction} onChange={(event) => setReviewAction(event.target.value)} placeholder="مثال: استلام اعتماد العميل للتصميم" /></div><div className="space-y-1"><Label>تاريخ الإكمال المتوقع</Label><Input type="date" value={reviewExpectedDate} onChange={(event) => setReviewExpectedDate(event.target.value)} /></div><div className="grid grid-cols-2 gap-3"><div className="space-y-1"><Label>يوجد عائق؟</Label><Select value={hasBlocker} onValueChange={setHasBlocker}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="no">لا</SelectItem><SelectItem value="yes">نعم</SelectItem></SelectContent></Select></div><div className="space-y-1"><Label>نوع التحديث</Label><Select defaultValue="status_update" onValueChange={() => undefined}><SelectTrigger><SelectValue placeholder="تحديث حالة" /></SelectTrigger><SelectContent><SelectItem value="status_update">تحديث حالة</SelectItem><SelectItem value="monday_review">مراجعة الاثنين</SelectItem><SelectItem value="wednesday_review">مراجعة الأربعاء</SelectItem></SelectContent></Select></div></div><div className="space-y-1"><Label>ملاحظات التحديث</Label><Textarea value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} placeholder="الحالة الحالية والعائق إن وجد…" /></div><Button className="w-full" disabled={reviewMutation.isPending} onClick={() => detailId && reviewMutation.mutate({ projectId: detailId, currentStatus: selectedProject.projectStatus, nextAction: reviewAction || undefined, expectedCompletionDate: reviewExpectedDate || undefined, hasBlocker: hasBlocker === "yes", notes: reviewNotes || undefined, updatedBy })}>{reviewMutation.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="ml-2 h-4 w-4" />}حفظ تحديث المشروع</Button></CardContent></Card>
                  <CurrentStageOverview project={selectedProject} stages={stages} engineers={engineers} />
                  <ScheduleUpdateCard project={selectedProject} updatedBy={updatedBy} onChanged={refresh} />
                  <ProjectCloseCard project={selectedProject} updatedBy={updatedBy} onChanged={refresh} />
                  <StageDelayCard project={selectedProject} owners={owners} reasons={reasons} updatedBy={updatedBy} onChanged={refresh} />
                  {!selectedProject.executionStartDate && <CompactPreExecutionPanel project={selectedProject} engineers={engineers} updatedBy={updatedBy} onChanged={refresh} />}
